...
let mainInputValue = mainSearchInput === null || mainSearchInput === void 0 ? void 0 : mainSearchInput.value;
let nbeRecettes;
/*******************************Tableaux***************************************** */
const recipeArray = [];
...
btnIngredient === null || btnIngredient === void 0 ? void 0 : btnIngredient.addEventListener("click", function (event) {
...
function constructRecipe(recipeData) {
    ...
    galerie === null || galerie === void 0 ? void 0 : galerie.appendChild(recipeCard);
}
function getRecipeData() {
    return __awaiter(this, void 0, void 0, function* () {
        const response = yield fetch("./data/recipes.json");
...
function advancedFilter(clearBtn, inputELEMENT) {
    ...
    if (inputValue.length > 2) {
        const filteredRecipes = recipeArray.filter(recipe => activeFilters.every(filterValue => recipe.ingredients.some((ingredient) => ingredient.ingredient === filterValue) ||
        ...
        if (filtreSectionDOM.children.length >= 1) {
            ...
            filteredRecipes.forEach(filteredRecipe => {
                ...
                filteredRecipes.forEach(recipe => {
                    recipe.ustensils.forEach((ustensil) => ustensileSet.add(ustensil));
                });
                filteredRecipes.forEach(recipe => {
                    applianceSet.add(recipe.appliance);
                });
        ...
        if (filtreSectionDOM.children.length >= 1) {
            ...
            const filteredRecipes = recipeArray.filter(recipe => recipe.ingredients.some((ingredient) => activeFilters.includes(ingredient.ingredient)) ||
                recipe.ustensils.some((ustensil) => activeFilters.includes(ustensil)) ||
...
getRecipeData().then(recipes => {
    ...
    constructFilterLists(ingredientSet, applianceSet, ustensileSet);
    /*************************************Création des tags**************************************** */
    filtresContainers.forEach(filtreContainer => filtreContainer.addEventListener("click", (event) => {
        ...
        if (target instanceof Element && target.classList.contains("filtre_list_item")) {
            ...
            if (activeFilters.length === 1 && filtreSectionDOM.children.length === 0) {
                ...
                const filteredRecipes = recipeArray.filter(recipe => recipe.ingredients.some((ingredient) => ingredient.ingredient.includes(mainInputValue)) ||
                    recipe.name.includes(mainInputValue) ||
                    recipe.description.includes(mainInputValue)).filter(recipe => recipe.ingredients.some((ingredient) => ingredient.ingredient === value) ||
            ...
            if (filtreSectionDOM.children.length >= 1 && mainInputValue == "") {
                ...
                filteredRecipes.forEach(filteredRecipe => {
                    ...
                    filteredRecipe.ingredients.forEach((ingredient) => {
                        const isIngredientActive = activeFilters.every(activeFilter => ingredient.ingredient !== activeFilter);
                        if (isIngredientActive) {
                            ingredientSet.add(ingredient.ingredient);
                        }
                    });
                    filteredRecipe.ustensils.forEach((ustensil) => {
                        const isUstensilActive = activeFilters.every(activeFilter => ustensil !== activeFilter);
                        if (isUstensilActive) {
                            ustensileSet.add(ustensil);
                        }
                    });
                    const isApplianceActive = activeFilters.every(activeFilter => filteredRecipe.appliance !== activeFilter);
                ...
                const filteredRecipes = recipeArray.filter(recipe => recipe.ingredients.some((ingredient) => ingredient.ingredient.includes(mainInputValue)) ||
                    recipe.name.includes(mainInputValue) ||
                    recipe.description.includes(mainInputValue)).filter(recipe => activeFilters.every(filterValue => recipe.ingredients.some((ingredient) => ingredient.ingredient === filterValue) ||
                    recipe.ustensils.some((ustensil) => ustensil === filterValue) ||
                    recipe.appliance === filterValue));
                galerie.innerHTML = "";
                setRecipeNumber(filteredRecipes);
            ...
            filtreSectionDOM.addEventListener("click", (event) => {
                const target = event.target;
